...
abstract class SubscriptionBackendDataSource {
  ...
  Future<FeatureEntitlementStatus?> fetchFeatureQuota({
...
class FirebaseSubscriptionBackendDataSource
  ...
  Future<FeatureEntitlementStatus?> fetchFeatureQuota({
    ...
    final payload = await _functionsService.call(
      'getRemainingQuota',
      payload: <String, dynamic>{'feature': feature.key},
    );
    ...
    return SubscriptionBackendMapper.mapFeatureAccess(
      payload,
      feature: feature,
    );
  }
...
class MockSubscriptionBackendDataSource implements SubscriptionBackendDataSource {
  ...
  Future<FeatureEntitlementStatus?> fetchFeatureQuota({
    ...
    return _snapshot.status.accessFor(feature);
  }
}
...
class SubscriptionBackendMapper {
  ...
  static FeatureEntitlementStatus mapFeatureAccess(
    Map<String, dynamic> payload, {
    required PremiumFeature feature,
  }) {
    final int quota = _coerceInt(payload['quota']);
    final int used = _coerceInt(payload['used']);
    final int remaining = _coerceInt(payload['remaining']);
    final String? reason = payload['reason']?.toString();
    final bool allowed = payload['allowed'] == true;
    final bool entitled = reason != 'feature_not_entitled';
